...
import { ApiErrorType } from '@chatvolt/lib/api-error';
import getS3RootDomain from '@chatvolt/lib/get-s3-root-domain';
import { hasAdminRole } from '@chatvolt/lib/has-oneof-roles';
  ...
  const submitInvite = async (values: OrganizationInviteSchema) => {
    try {
      if (
        ...
        alert(
          'You have reached the maximum number of seats for your plan. Please contact support@chatvolt.ai to upgrade your plan.'
        );
        return;
      }
      ...
      if (
        ...
        toast.error(
          "Unable to send invite, please contact support@chatvolt.ai if it's persistent.",
          {
            duration: 5000,
          }
        );
