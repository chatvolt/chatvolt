...
import formatSourcesRawText from '@chatvolt/lib/form-sources-raw-text';
import handleChatMessage, {
  ChatAgentArgs,
  ChatConversationArgs,
} from '@chatvolt/lib/handle-chat-message';
import i18n from '@chatvolt/lib/locales/i18next';
import {
  Action,
  AIStatus,
  ConversationMetadata,
} from '@chatvolt/lib/types/crisp';
...
import getRequestLocation from '@chatvolt/lib/get-request-location';
...
//     user: {
//       type: 'participant',
//       nickname: agentName || 'Chatvolt',
//       avatar: 'https://chatvolt.ai/logo.png',
//     },

//     content: {
//       id: `chatvolt-query-${cuid()}`,
//       text: `✨ Ask ${agentName || `Chatvolt`}`,
//       explain: 'Query',
//       value,
//     },
//   });
...
const handleQuery = async ({
  ...
  if (chatResponse?.agentResponse) {
    ...
    await CrispClient.website.sendMessageInConversation(websiteId, sessionId, {
      ...
      user: {
        type: 'participant',
        nickname: agent?.name || 'Chatvolt',
        avatar: agent?.iconUrl || 'https://chatvolt.ai/logo.png',
      },
    });
  }
};
